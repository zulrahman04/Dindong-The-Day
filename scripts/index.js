...
const config = {
  birthdate: 'Nov 23, 2022',
  name: 'Artiani Chandra N.'
};
...
x = setInterval(function() {
  ...
  let baseWidth = 1920,
    ...
    hw = w / 2, // half-width
    hh = h / 2,
    scale = Math.min(w / baseWidth, h / baseHeight) * 0.7,
    opts = {
      strings: ['HAPPY', 'BIRTHDAY!', config.name],
      charSize: Math.floor(90 * scale),
      charSpacing: Math.floor(95 * scale),
      lineHeight: Math.floor(100 * scale),
  ...
  function resizeCanvas() {
    ...
    if (w < h) {
      // Mobile, tambahkan skala lebih besar agar teks lebih jelas
      scale = Math.min(w / baseWidth, h / baseHeight) * 1.2; // Skala lebih besar
    } else {
      // Desktop
      scale = Math.min(w / baseWidth, h / baseHeight) * 0.7; // Skala lebih kecil
    }
    opts.charSize = Math.floor(90 * scale);
    opts.charSpacing = Math.floor(95 * scale);
    opts.lineHeight = Math.floor(100 * scale);
